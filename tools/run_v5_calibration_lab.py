...
def main():
    ...
    db=root/"research_data/v5/replay/databento_v5_evidence.db";out=root/"research_data/v5/calibration"
    ...
    resolved=eligible[eligible.outcome.isin(["stop_first","target_3r_first"])].copy()
    ...
    report={"schema":"TP_V5_CHRONOLOGICAL_CALIBRATION_AUDIT_1","generated_utc":datetime.now(timezone.utc).isoformat(),
    ...
    rp=out/"v5_calibration_audit_report.json";rp.write_text(json.dumps(report,indent=2,default=str),encoding="utf-8")
    print("Phase 3/3 complete: holdout reported and compact result files written",flush=True)
    print("Trading Pulse V5 Chronological Calibration Audit")
