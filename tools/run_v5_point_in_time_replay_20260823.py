...
SCHEMA = "TP_DATABENTO_V5_POINT_IN_TIME_REPLAY_2"
...
def available_at_close(series, duration):
    """Move left-labeled candle values to the first timestamp they were knowable."""
    out = series.copy()
    out.index = out.index + pd.Timedelta(duration)
    return out
...
def generate_candidates(symbol, m15, h1, h4, daily):
    ...
    # Warehouse candles are left-labeled.  Shift every higher-timeframe value to
    # its close time so no unfinished candle can leak into a candidate.
    daily_atr_known = available_at_close(d.atr, "1D")
    h1_ma = available_at_close(h1.close.rolling(20, min_periods=20).mean(), "1h")
    h4_ma = available_at_close(h4.close.rolling(20, min_periods=20).mean(), "4h")
    h1_close = available_at_close(h1.close, "1h")
    h4_close = available_at_close(h4.close, "4h")
    rows = []
    ...
    for i in range(30, len(x)-3):
        ...
        if direction is None:
            continue
        # i+3 is the third departure candle's START. Its OHLC is not knowable
        # until the 15-minute candle closes.
        detected = x.index[i+3] + pd.Timedelta(minutes=15)
        daily_atr = prior_asof(daily_atr_known, detected)
        ...
        c1, c4 = prior_asof(h1_close, detected), prior_asof(h4_close, detected)
...
def replay(rows, raw, max_minutes=14400):
    ...
    for n, r in enumerate(rows, 1):
        # The order becomes active at detection. Include the one-minute candle
        # beginning at that timestamp; ambiguous entry/exit ordering remains
        # conservatively labeled rather than guessed.
        start = idx.searchsorted(pd.Timestamp(r["detected_at"]), side="left")
...
def main():
    ...
    out=v5/"replay_point_in_time"; out.mkdir(parents=True,exist_ok=True); db=out/"databento_v5_evidence_point_in_time.db"
